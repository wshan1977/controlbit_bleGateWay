...
[UWB 앵커 보드(PCB v1.2)](https://wshan1977.github.io/controlbit_uwb/)와 동일 PCB · 동일 케이스를 사용하는 BLE 게이트웨이 파생 모델입니다.
UWB 모듈 · OLED · 스위치를 미실장하고 BLE 스캔 게이트웨이 펌웨어를 탑재합니다.
...
| 항목 | 값 |
| ---- | ---- |
| PCB | UWB 앵커 v1.2와 동일 (UWB 모듈 · OLED 커넥터 · 스위치 미실장) |
| 케이스 재질 | 플라스틱, 앵커와 동일 외관 |
| 마운팅 | 후면 브라켓 (천장/벽 부착) |
...
UWB 송신이 없어 앵커 대비 평균 소비 전류는 낮습니다. 수치는 실측 후 갱신합니다.
...
UWB 미실장으로 6.5 GHz 대역 인증 항목이 빠집니다. 앵커와 별도 인증이 필요합니다.
